...
import { ArrowLeft, ChefHat, Clock, UtensilsCrossed, Users } from 'lucide-react';
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    title: recipe?.title ? `${recipe.title as string} — Recipe` : 'Recipe',
  };
}
...
export default async function RecipeDetailPage({ params }: PageProps) {
  ...
  // 2. Fetch the recipe (scoped to the current user)
  const { data: recipe } = await supabase
    .from('recipes')
    .select(
      'id, title, description, cuisine_type, prep_time_minutes, cook_time_minutes, servings, difficulty, status, cover_image_url, is_public',
    )
    .eq('id', id)
    .eq('user_id', user.id)
    .maybeSingle();
      ...
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <ShareButton
              ...
              initialIsPublic={recipe.is_public as boolean}
            />
            <Link
              href={`/recipes/${recipe.id as string}/edit`}
              className="flex items-center gap-2 rounded-xl bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-zinc-700"
            >
              Edit
            </Link>
          </div>
        ...
        <header className="mb-8 space-y-4">
          ...
          {totalTime > 0 && (
            <div className="flex flex-wrap gap-3">
              ...
              {cookTime > 0 && (
                <TimeTile
                  icon={<ChefHat className="h-4 w-4 text-zinc-500" />}
                  label="Cook"
                  value={`${cookTime} min`}
                />
              )}
        ...
        {hasIngredients && (
          <section className="mb-8">
            <h2 className="mb-4 text-xl font-semibold text-zinc-900">Ingredients</h2>
            <ul className="divide-y divide-zinc-100 overflow-hidden rounded-2xl border border-zinc-200 bg-white">
              {(ingredients ?? []).map((ing) => (
                ...
                >
                  <span className="text-sm text-zinc-800">{ing.name as string}</span>
        ...
        {hasInstructions && (
          <section className="mb-12">
            <h2 className="mb-4 text-xl font-semibold text-zinc-900">Instructions</h2>
